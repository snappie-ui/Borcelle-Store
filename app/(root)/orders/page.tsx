...
const Orders = async () => {
  ...
  return (
    <div className="px-10 py-5 max-sm:px-3">
      ...
      {/* Corrected "No Orders" logic */}
      {!orders || orders.length === 0 ? (
        <p className="text-body-bold my-5">You have no orders yet.</p>
      ) : (
        <div className="flex flex-col gap-10">
          {orders.map((order: OrderType) => (
            // FIX 1: Added unique key to the outer loop element
            <div key={order._id} className="flex flex-col gap-8 p-4 hover:bg-grey-1">
              ...
              <div className="flex flex-col gap-5">
                {order.products.map((orderItem: OrderItemType) => (
                  // FIX 2: Added unique key to the inner loop element
                  <div key={orderItem.product._id} className="flex gap-4">
                    <Image
                      // FIX 3: Safely access the image source
                      src={orderItem.product.media?.[0] || "/placeholder.png"}
